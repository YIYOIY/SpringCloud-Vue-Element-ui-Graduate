  ...
  <div id="building">
    ...
    <div v-show="!havePicture" style="right:1%;bottom: 60%;width:40%;position: absolute">
      <el-upload ref="pict" class="upload-demo" :action="`picture/test/up/${store.state.adminId}`" multiple
        ...
        <template #tip>
          <div class="el-upload__tip" style="color: #f5f2f2;font-size: 15px">
...
import {alterAdmin, getAdmin} from "@/api/AdminApi";
...
onMounted(async () => {
  await getAdmin(id.value).then((Response) => {
...
#building {
  ...
  background: url(../../assets/static/userSelf.jpg) no-repeat fixed bottom;
  z-index: -1;
}
