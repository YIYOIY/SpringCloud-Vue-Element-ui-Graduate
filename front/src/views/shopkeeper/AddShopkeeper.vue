  ...
  <div id="building">
    <el-form label-position="top" ref="form" :model="shopkeeper.shopkeeper" :rules="rule">
      ...
      <div style="left: 2%;width:35%;height:20%;position: relative">
        <el-form-item label="账户名称" prop="shopkeeperName">
          <el-input v-model="shopkeeper.shopkeeper.shopkeeperName" autofocus @change="next" maxlength="20" minlength="1" clearable
        ...
        <el-form-item label="联系方式" prop="shopkeeperPhone">
          <el-input v-model.number="shopkeeper.shopkeeper.shopkeeperPhone" clearable maxlength="11" @change="next"
                    placeholder="联系方式作为登录凭证"></el-input>
        </el-form-item>
      </div>
      ...
      <div style="left: 30%;top:20%;width: 15%;position: absolute">
        <el-form-item label="注册日期">
          <el-date-picker clearable v-model="shopkeeper.shopkeeper.shopkeeperBirth"
                          type="date" format="YYYY 年 MM 月 DD 日" @change="next" placeholder="在此选择注册日期">
          </el-date-picker>
        </el-form-item>
      ...
      <div style="left: 12%;top:50%;width: 30%;position: absolute">
        <el-form-item label="地址" prop="shopkeeperAddress" @change="next">
    ...
    <div style="right: 2%;width: 50%;top:5%;position: absolute">
      <el-row>
        ...
        <el-row  v-show="havePicture">
          <el-col :span="24">
            <el-image style="width: 780px; height: 580px;margin-top: 5%;z-index: 2" :src="shopkeeper.shopkeeper.image.picture" fit="contain" v-show="havePicture"/>
          </el-col>
    ...
    <div style="left: 5%;width:50%;bottom:10px;position: absolute">
      <el-steps :active="active" align-center finish-status="success"
        ...
        <el-step title="Step 1" description="设置账号名"/>
        <el-step title="Step 2" description="设置密码"/>
...
import {addShopkeeper} from "@/api/ShopkeeperApi";
...
console.log("是否从首页进入而非企业管理页进入" + isEnroll.value)
  ...
  setTimeout(() => {
    getPicture(id).then(Response => {
      ...
      if (Response.data != null && Response.data !== "") {
        //对后端返回值验证后进行赋值
        shopkeeper.shopkeeper.image.picture = Response.data
        // 显示图片页面
        console.log(shopkeeper.shopkeeper.image.picture + "数据库保存的图片地址")
...
const add = (() => {
  form.value.validate(valid => {
    ...
    let Shopkeeper = JSON.stringify(shopkeeper.shopkeeper)
    console.log("要保存的企业对象信息" + Shopkeeper)

    addShopkeeper(Shopkeeper).then(Response => {
      if (isEnroll.value) {
        ElNotification({
