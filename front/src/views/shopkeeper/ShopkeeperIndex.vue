<template>
  <div id="building">
    ...
    <div v-show="!havePicture" style="right:1%;bottom: 60%;width:40%;position: absolute">
      <el-upload ref="pict" class="upload-demo" :action="`picture/test/up/${store.state.shopkeeperId}`" multiple
                 :limit="1"
        ...
        <template #tip>
          <div class="el-upload__tip" style="color: #1e1c1c;font-size: 15px">
    ...
    <div style="left: 8%;top:75%;width:40%;position: absolute">
      <el-row :gutter="40">
        ...
        <el-col :span="12">
          <el-button type="danger" size="large" @click="removeShopkeeper()" plain round>注销账号</el-button>
        </el-col>
      </el-row>
    </div>
...
import {alterShopkeeper, deleteShopkeeper, getShopkeeper} from "@/api/ShopkeeperApi";
...
let handleBookPicture = (() => {
  ...
  setTimeout(() => {
    getPicture(store.state.shopkeeperId).then(Response => {
...
// 加载时先根据企业id获取企业个人信息
onMounted(async () => {
  console.log(store.state.shopkeeperId)
  await getShopkeeper(store.state.shopkeeperId).then(Response => {
...
let removeShopkeeper = (() => {
  if (confirm("确认注销?")) {

    let Shopkeeper = JSON.stringify(shopkeeper.shopkeeper)
    console.log("要注销的的企业对象信息" + Shopkeeper)

    deleteShopkeeper(Shopkeeper).then(Response => {
      ElNotification({
        message: Response.message,
        title: '成功！',
        type: 'success',
...
#building {
  ...
  background: url(../../assets/static/userSelf.jpg) no-repeat fixed top;
  z-index: -1;
}
