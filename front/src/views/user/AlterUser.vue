    ...
    <el-form label-position="top" ref="form" :model="user.user" :rules="rule">
      ...
      <div style="left: 30%;top:20%;width: 15%;position: absolute">
        ...
        <el-form-item label="账户余额">
          <el-tag v-model.number="user.user.userMoney" type="danger" >{{user.user.userMoney>0?user.user.userMoney:0}}元</el-tag>
        </el-form-item>
...
import {alterUser, getUser} from "@/api/UserApi";
...
onMounted(async () => {
  await getUser(id.value).then((Response) => {
...
let alter = (() => {
  form.value.validate(valid => {
    ...
    user.user.image.picture = PICTURE.value
    let obj = JSON.stringify(user.user)
    console.log("要修改的的用户对象" + obj)

    alterUser(obj).then(Response => {
      ElNotification({
...
#building {
  ...
  background: url(../../assets/static/star.jpg) no-repeat fixed center;
  z-index: -1;
}
