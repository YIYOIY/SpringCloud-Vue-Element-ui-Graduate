...
public class ExcelController {
    ...
    public void export(HttpServletResponse response) {
        List<Book> all = bookMapper.getAllByExcel();
        String[] titles = {"书籍ID", "书籍编号", "书名", "作者", "封面", "出版社", "价格", "出版日期", "系列名"};
        String[][] value = new String[all.size()][titles.length];
        for (int i = 0; i < all.size(); i++) {
            value[i][0] = String.valueOf(all.get(i).getBookId());
            value[i][1] = String.valueOf(all.get(i).getBookNo());
            value[i][2] = all.get(i).getBookName();
            value[i][3] = all.get(i).getBookAuthor();
            value[i][4] = all.get(i).getBookPicture();
            value[i][5] = all.get(i).getBookFactory();
            value[i][6] = String.valueOf(all.get(i).getBookPrice());
            value[i][7] = String.valueOf(all.get(i).getBookAddDate());
            value[i][8] = all.get(i).getSeriesName();
        }
...
//                此处可以修改便利导入数据库，注意sql命令并没有补全，目前仅仅是作为一个测试用的sql，有很大的问题
                    bookMapper.insertAll(e.getBookName());
            });
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private List<Book> readExcelValue(Workbook workbook) {
        ...
        for (int rowNum = 2; rowNum < totalRows; rowNum++) {
            ...
            String[] titles = {"bookId", "bookNo", "bookName", "bookAuthor", "bookPicture", "bookFactory", "bookPrice", "bookAddDate", "seriesName"};
            Book book = new Book();
            // 逐列取数据，每列对应一个实体对象属性
